...
# Script to seed demo data for InvoiceMe LOCAL development
# This script creates sample customers, invoices, and payments for local testing
...
echo "======================================"
echo "InvoiceMe Local Data Seeding Script"
echo "======================================"
echo ""
...
for i in {1..30}; do
    if curl -s -f "$BACKEND_URL/actuator/health" > /dev/null 2>&1 || \
       curl -s -f "$BACKEND_URL/swagger-ui/index.html" > /dev/null 2>&1; then
        echo "Server is ready!"
        break
    fi
    if [ $i -eq 30 ]; then
        echo -e "${RED}Server is not responding. Please start the backend first.${NC}"
...
echo "======================================"
echo -e "${GREEN}Local data seeding completed!${NC}"
echo "======================================"
echo ""
echo "Summary:"
